...
CREATE TABLE `role_transaction_type_authorization` (
  `id_role_transaction_type_authorization` int(10) NOT NULL auto_increment,
  `role_id` int(10) unsigned NOT NULL,
  ...
  `modified_date` timestamp NULL default NULL on update CURRENT_TIMESTAMP,
  PRIMARY KEY  (`id_role_transaction_type_authorization`),
  ...
  KEY `role_transaction_type_authorization_UNIQUE` (`role_id`,`transaction_type_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
ALTER TABLE `role_transaction_type_authorization`
  ADD CONSTRAINT `role_transaction_type_authorization_ibfk_5` FOREIGN KEY (`role_id`) REFERENCES `role` (`role_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
