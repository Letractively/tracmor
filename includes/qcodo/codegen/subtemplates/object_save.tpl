...
<% } %><%-----%>
						...
						if ($objRow[0] != $this-><%= $objColumn->VariableName %>)
							throw new QExtendedOptimisticLockingException('<%= $objTable->ClassName %>'<% foreach ($objTable->PrimaryKeyColumnArray as $objPkColumn) { %><% if ($objPkColumn->Identity) { %>, $this-><%= $objPkColumn->VariableName %><% } %><% } %>);
					}
	<% } %>
<% } %>
